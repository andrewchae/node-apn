...
Connection.prototype.createConnection = function() {
	...
	this.connect().then(function () {
		...
		if (this.options.enhanced && this.options['ackTimeout']) {
			...
			var ackTimeout = setTimeout(doAckTimeout,this.options['ackTimeout']);
			sock.once('data',function(){
				clearTimeout(ackTimeout);
			});
			function doAckTimeout (){
			 	if ( sock.cachedNotifications.length ) {
					self.transmitNotification(sock, { 
						"notification": new Notification(), 
						"recipient": new Device('aaaa1111'), 
						"retryLimit": 0 
					});
			 	} else {
			 		ackTimeout = setTimeout(doAckTimeout,this.options['ackTimeout']);
				}
			};
		}
